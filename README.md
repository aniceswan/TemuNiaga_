...
- Harga (BPS webapi) butuh `BPS_API_KEY` di `apps/api/.env` plus `domain`/`varId`/`th` (tahun) dataset spesifik per komoditas (dipilih dari akun BPS Anda) lewat `POST /harga/refresh` — lihat `apps/api/src/harga/bps-client.ts`. Diverifikasi dengan key asli: webapi.bps.go.id ada di balik WAF yang menolak request tanpa header `User-Agent` browser (sudah ditangani di client).
